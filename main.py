# -*- coding: utf-8 -*-
"""
Created on Sat Mar 21 18:32:34 2020
...
variable = ea.bs2int(Chrom, FieldD) # 对初始种群进行解码
for v1 in range(NIND):
        if len(variable[v1,:][variable[v1,:]>0])<12:
          va = np.argwhere(variable[v1,:] < 1)
          vd=va[0:(12-(len(variable[v1,:][variable[v1,:]>0])))]
          variable[v1,vd]=1
        else:
            if len(variable[v1,:][variable[v1,:]>0])>12:
             vQ = np.argwhere(variable[v1,:]==1)
             v2=vQ[0:((len(variable[v1,:][variable[v1,:]>0]))-12)]
             variable[v1,v2]=0
...
for gen in range(MAXGEN):
    ...
    Phen = ea.bs2int(Chrom, FieldD) # 对种群进行解码(二进制转十进制)
    for v in range(NIND):
        if len(Phen[v,:][Phen[v,:]>0])<12:
          indexP = np.argwhere(Phen[v,:] < 1)
          Change1=indexP[0:(12-(len(Phen[v,:][Phen[v,:]>0])))]
          Phen[v,Change1]=1
        else:
            if len(Phen[v,:][Phen[v,:]>0])>12:
             indexQ = np.argwhere(Phen[v,:]==1)
             Change2=indexQ[0:((len(Phen[v,:][Phen[v,:]>0]))-12)]
             Phen[v,Change2]=0
...
variable = ea.bs2int(var_trace[[best_gen], :], FieldD) # 解码得到表现型（即对应的决策变量值）
print('最优解的决策变量值为：')
