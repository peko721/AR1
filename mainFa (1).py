...
"""
import pandas as pd
sall=pd.read_excel('C:\\Users\\Administrator\\Desktop\\Re.xlsx', index_col=None, na_values=['NA'])
Me=pd.read_excel('C:\\Users\\Administrator\\Desktop\\Measurement.xlsx', index_col=None, na_values=['NA'])
Del_Measure=[]
for k in range(0,len(Me.loc[:,'R'])):
    Del_Measure.append(0.001)     #%对应波点光谱反射率容差 
import numpy as np
...
ObjV, CV = aimfunc(variable, CV,sall,Me,Del_Measure) # 计算初始种群个体的目标函数值
...
for gen in range(MAXGEN):
    ...
    ObjV, CV = aimfunc(Phen, CV,sall,Me,Del_Measure) # 求种群个体的目标函数值
    FitnV = ea.ranking(maxormins * ObjV, CV) # 根据目标函数大小分配适应度值
    # 记录
    best_ind = np.argmax(FitnV) # 计算当代最优个体的序号
